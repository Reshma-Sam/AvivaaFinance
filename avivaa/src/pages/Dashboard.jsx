...
export default function Dashboard() {
  ...
  const handleSaveAccountNumber = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/loans/${selectedLoan._id}/account-number`, {
        method: "PUT",
        headers: {
  ...
  const handlePdfUpload = async (e) => {
    ...
    try {
      ...
      reader.onload = async () => {
        ...
        setLoans(loans.map(loan => loan._id === selectedLoan._id ? updated : loan));
        setSelectedLoan(updated);
        showAlert("PDF uploaded and linked to user successfully!", "success");
      };
    } catch (err) {
      showAlert(err.message, "error");
    } finally {
  ...
  const handleDeleteLoan = (loanId) => {
    showConfirm(
      "Confirm Deletion",
      "Are you sure you want to permanently delete this application record? This action cannot be undone and will delete all associated media files from Cloudinary storage.",
      async () => {
        ...
        try {
          ...
          showAlert("Application record and associated files deleted successfully.", "success");
        } catch (err) {
          showAlert(err.message, "error");
        } finally {
          setDeletingId(null);
                        ...
                        {isEditingAccount ? (
                          ...
                          <div className="flex items-center justify-between mt-1">
                            <span className="text-sm font-mono font-bold text-white tracking-widest">
                              {selectedLoan.bankDetails.accountNumber}
                            </span>
                            <button
